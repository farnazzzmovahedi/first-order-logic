...
visit_all_pigs_graph(Start, [NextPig | RemainingPigs], Path) :-
    bfs(Start, NextPig, SubPath),  % Find path to the next pig
    writeln(['Path to pig:', SubPath]),
    visit_all_pigs_graph(NextPig, RemainingPigs, RemainingPath),  % Continue visiting remaining pigs
    append(SubPath, RemainingPath, Path).
...
solve :-
    ...
    visit_all_pigs_graph((BirdX, BirdY), Pigs, Path),
